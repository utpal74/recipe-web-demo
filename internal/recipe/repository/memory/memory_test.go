...
func TestRepositoryGetByID(t *testing.T) {
	...
	data, _ := json.MarshalIndent([]domain.Recipe{recipe}, "", " ")
	os.WriteFile(tempFile, data, 0644)
...
func TestRepositoryUpdate(t *testing.T) {
	...
	// Not found
	_, err = repo.Update(context.Background(), domain.Recipe{ID: "nonexistent"})
	if err != ErrNotFound {
		t.Error("Expected ErrNotFound")
	}
}
...
func TestRepositoryConcurrency(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func(i int) {
			recipe := domain.Recipe{Name: fmt.Sprintf("Recipe %d", i)}
